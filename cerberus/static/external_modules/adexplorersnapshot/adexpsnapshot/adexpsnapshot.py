from .parser.classes import Snapshot
...
def parseDatFile(snapshot, output):
    ...
    if not os.path.exists(output):
        try:
            ...
            #log.error(f"Unable to create output directory '{output}'.")
            return
    
    if not os.path.isdir(output):
        #log.warn(f"Path '{output}' does not exist or is not a folder.")
        return
    
    ades = ADExplorerSnapshot(snapshot, output)
    try:
        ades.outputBloodHound()
    except Exception as e:
        print("ERROR: The snapshot has not been correctly parsed, you may encounter incomplete information")
